 ...
 * CONSIDERACOES:
 * MovingSafePlace:
 *	Checar se o vetor r2 foi definido corretamente.
 *	Assumindo D como a distancia euclidiana.
 *	Assumindo Theta como um angulo escolhido aleatoriamente de uma distribuicao normal entre -pi/6 e pi/6 (encontrado na secao 2.2.4 do paper).
 * 
 * Roaming:
 * 	verificar se sao escolhidos varios territorios e o leao vai ate cada um deles a partir do ultimo ou se eh escolhido apenas um territorio e o leao vai ate ele.
 * 
 * NomadMovingRandom:
 *	Assumindo Best como a melhor posicao daquele determinado nomade, nao a melhor posicao com relacao a todos os nomades.
 */

/**
 * In each pride, some females look for a prey (hunt).
 * @param s                search space.
 * @param pride            pride identification number.
 * @param selected_females binary array (zeroed) that each index indicates if a female is hunting right now or not.
 * @param Evaluate         pointer to the function used to evaluate lionsses.
 * @param arg              list of additional arguments.
 */
void Hunting(SearchSpace *s, int pride, int *selected_females, prtFun Evaluate, va_list arg){
  ...
  for(i = 0; i < s->pride_id[pride].n_females; i++){
    ...
    if(selected_females[i]){ /* if ith female was selected */
 ...
 * @param s                search space.
 * @param pride            pride identification number.
 * @param selected_females binary array declared (zeroed) in'runLOA()' and modified on 'hunting()' that each ith element indicates if the ith female is hunting right now or not.
 * @param Evaluate         pointer to the function used to evaluate lionsses.
 * @param arg              list of additional arguments.
 */
 ...
 * @param s        search space.
 * @param pride    pride identification number.
 * @param Evaluate pointer to the function used to evaluate lions.
 * @param arg      list of additional arguments.
 */
 ...
 * @param s                 search space.
 * @param extra_male_nomads number of extra male nomads that moved out from the prides.
 * @param Evaluate          pointer to the function used to evaluate male and female lions.
 * @param arg               list of additional arguments.
 */
 ...
 * @param s            search space.
 * @param pride        pride identification number.
 * @param n_offsprings number of offsprings that will be generated by matting.
 * @param Evaluate     pointer to the function used to evaluate the cubs.
 * @param arg          list of additional arguments.
 */
 ...
 * @param s                 search space.
 * @param n_offsprings      number of offsprings that will be generated by nomad matting.
 * @param extra_male_nomads number of extra male nomads that moved out from the prides.
 * @param Evaluate          pointer to the function used to evaluate the cubs.
 * @param arg               list of additional arguments.
 */
 ...
 * Basically, we sort all males (old and new ones) of the pride, the excess of males become nomads
 * @param s                 search space.
 * @param pride             pride identification number.
 * @param n_male_offsprings number of male offsprings generated by matting (half of total offsprings).
 * @param extra_male_nomads number of extra male nomads that moved out from the prides.
 */
 ...
 * @param s                      search space.
 * @param n_nomad_male_offspring number of male offsprings generated by nomad matting (half of total offsprings).
 * @param extra_male_nomads      number of extra male nomads that moved out from the prides.
 */
 ...
 * s->imigration_rate percent of females on pride plus the surplus of females (female offsprings) will become nomads
 * @param s                        search space.
 * @param n_offsprings             array that each index contains the number of offsprings of each pride generated by mating.
 * @param n_nomad_female_offspring number of female offsprings generated by nomad matting (half of total offsprings).
 * @param extra_nomads             number of extra female nomads after pride females become nomads.
 * @param n_migrating              array that each index contains the number of females that will migrate from each pride.
 */
  ...
  *extra_nomads = 0;
  ...
  for(i = 0; i < s->n_prides; i++){
    ...
    for(j = 0; j < s->pride_id[i].n_females + (int)(n_offsprings[i] / 2); j++){
      if(selected[j]){ /* if the female was selected for migration */
         s->female_nomads[nomad_index] = s->pride_id[i].females[j]; /* female become nomad */
         nomad_index++;
      }
      else{ /* copy the pride female to the new array of females of the pride */
 ...
 * @param s        search space.
 * @param Evaluate pointer to the function used to evaluate the cubs.
 * @param VARARGS  list of additional arguments.
 */
