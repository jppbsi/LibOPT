...
void ImproveBirds(SearchSpace *s){
    ...
    for(i = 0; i < s->m; i++){ /* for each bird */
        if(s->a[i]->nb[0]->fit < s->a[i]->fit){ /* It compares the current bird with its best neighbour */
            for(j = 0; j < s->n; j++)
...
void runMBO(SearchSpace *s, prtFun Evaluate, ...){
    ...
    for(t = 1; t <= s->iterations; t++){
        ...
        for(tour = 0; tour < s->M; tour++){
            ...
            for(k = 1; k < s->m; k++){ /* It generates the k-X neighbours of each bird */
