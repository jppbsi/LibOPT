...
int main()
{
    ...
    s = ReadSearchSpaceFromFile("examples/model_files/hs_model.txt", _HS_); /* It reads the model file and creates a search space. We are going to use IHS to solve our problem. */
    ...
    if (CheckSearchSpace(s, _HS_))                 /* It checks whether the search space is valid or not */
        runTensorIHS(s, _QUATERNION_, Rosenbrock); /* It minimizes function Rosenbrock */
